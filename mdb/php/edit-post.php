...
if (isset($_POST['key'])) {
    ...
    try {
        ...
        if($_POST['key'] == "get_post_details"){
            ...
            $sql = "SELECT posts.id,posts.title,posts.titledetails,posts.description,posts.moredescription,posts.created_at,posts.updated_at, posts.status, users.firstname, users.lastname,users.about, posts.category, posts.created_at FROM (posts INNER JOIN users ON users.id=posts.author) WHERE posts.id=$postid";
            $posts = executer($sql);
            if(count($posts)>0){
                $post = $posts[0];
                $status = $post['status'];
            ...
            // perform an update 
            $sql = "UPDATE posts SET title='$title', titledetails = '$detailsTitle', `description`= '$detailsDescription', moredescription = '$detailsMoreDescription', category = '$category', `status`='$status', updated_at = current_timestamp WHERE id = $postid";
            if(noResultQuery($sql)=='done'){
                if(!empty($quote) ){
